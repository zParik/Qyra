...
impl PdfReader {
    /// Construct from an owned byte buffer (small inputs, tests, generated bytes).
    pub fn new(data: Vec<u8>) -> Result<Self, PdfError> {
        Self::from_source(Bytes::Owned(data))
    }

    /// Construct by memory-mapping a file — keeps large inputs off the heap.
    pub fn open(path: &Path) -> Result<Self, PdfError> {
        Self::from_source(Bytes::open(path)?)
    }

    /// Construct from any byte source (owned or memory-mapped).
    pub fn from_source(data: Bytes) -> Result<Self, PdfError> {
    ...
    /// Drop a cached object to release its memory. Streams (especially image
    /// XObjects) are large; the placement/plan pre-pass reads a stream's dict
    /// then evicts it so the cache never holds every image at once.
    pub fn uncache(&mut self, id: ObjectId) {
